...
public:
  void patrat(double lungime, int nivel, CPunct &p, CVector v)
  {
    ...
    else
    {
        ...
        //std::cout<<x<<" "<<y<<std::endl;
        v.rotatie(180);
        std::cout<<"Desenat patrat mare"<<std::endl;
        for (int i=0;i<3;i++)
            {
                for (int j=0;j<3;j++)
                    {
                        ...
                        cout<<"drawing "<<i<<" "<<j<<" "<<nivel-1<<endl;
                        p.getxy(x,y);
                        std::cout<<std::fixed<<std::setprecision(10);
                        std::cout<<x<<" "<<y<<std::endl;
                        this->patrat(lungime/3,nivel-1,p,v);
                        p=v.getDest(p,lungime/3);
                    }
                v.rotatie(180);
                p=v.getDest(p,lungime);
...
//sursa: https://en.wikipedia.org/wiki/Sierpi%C5%84ski_curve
class Arrow
{
public:
  void arrow(double lungime, int nivel, CPunct &p, CVector &v, int unghi)
  {
    if (nivel == 0) 
    {
      v.deseneaza(p, lungime);
    ...
    {
      arrow(lungime/2, nivel-1,p,v,-unghi);
      v.rotatie(unghi);
      arrow(lungime/2, nivel-1,p,v,unghi);
      v.rotatie(unghi);
      arrow(lungime/2, nivel-1,p,v,-unghi);
...
public:
  ...
  int isIn(CComplex &c)
  {
    int rez = 0;
    // an array for storing the values for computing z_n+1 = z_n * z_n + c
    CComplex z0,z1;//ambele sunt intializ cu 0

    for (int i = 1; i < m.nriter; i++)
    {
      z1 = z0 * z0 + c;
      ///std::cout<<z1.getModul()<<std::endl;
      if (z1.getModul() > 2)
      {
        // x does not belong to the J-F set because the 
        // iterative process converges infinitely
  ...
  void display(double xmin, double ymin, double xmax, double ymax)
  {
    ...
    for(double x = xmin; x <= xmax; x+=incr)
      ...
      {
        CComplex z(x, y);
        //std::cout<<z.getRe()<<" "<<z.getIm()<<std::endl;
        int r = isIn(z);
        if (r == 0) 
        {
          glColor3f(1.0, 0.1, 0.1);
          glVertex3d(x/xmax,y/ymax,0);
        }
        else if (r <= 5)
        {
            glColor3f(0, 0, 0);
            glVertex3d(x/xmax,y/ymax,0);
            //fprintf(stdout,"NOT MANDELBROT\n");
        }
      }
    fprintf(stdout, "STOP\n");
    glEnd();
